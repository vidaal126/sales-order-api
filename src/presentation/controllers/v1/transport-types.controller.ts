...
export class TransportTypesController {
  ...
  async create(@Body() dto: CreateTransportTypeDto): Promise<TransportTypeEntity> {
    return this.createTransportTypeUseCase.execute({ name: dto.name, description: dto.description });
  }

  @Put(':id')
  @ApiOperation({ summary: 'Atualizar tipo de transporte' })
  async update(@Param('id') id: string, @Body() dto: UpdateTransportTypeDto): Promise<TransportTypeEntity> {
    return this.updateTransportTypeUseCase.execute({ id, name: dto.name, description: dto.description });
  }
