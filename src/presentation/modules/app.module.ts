    ...
    LoggerModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => ({
        ...
        pinoHttp: {
          ...
          customLogLevel: (_req: IncomingMessage, res: ServerResponse, err: Error | undefined): LevelWithSilent => {
            if (res.statusCode >= 500 || err) return 'error';
            if (res.statusCode >= 400) return 'warn';
            return 'info';
          },
          autoLogging: { ignore: (req: IncomingMessage): boolean => req.url === '/health' },
          serializers: {
          ...
          customProps: (req: IncomingMessage): Record<string, unknown> => ({
            ...
            route: (req as IncomingMessage & { routeOptions?: { url?: string } }).routeOptions?.url ?? req.url,
          }),
