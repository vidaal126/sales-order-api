...
export class ScheduleDeliveryUseCase {
  ...
  async execute(input: ScheduleDeliveryInput): Promise<SchedulingEntity> {
    return this.unitOfWork.execute(async (transaction): Promise<SchedulingEntity> => {
      ...
      await this.outboxRepository.enqueue(
        ...
        {
          orderId: createdScheduling.salesOrderId,
          schedulingId: createdScheduling.id,
          deliveryDate: createdScheduling.deliveryDate,
        },
        transaction,
      );
